...
st.set_page_config(page_title="Deckle Optimizer", layout="wide")
...
# =====================================================
# FAST PATTERN GENERATION (CACHED + LIMITED)
# =====================================================
...
def generate_patterns(selected_items, repo):
    ...
    for i in range(n):
        ...
        for j in range(i, n):
            ...
            for k in range(j, n):
                ...
                triplet = tuple(sorted([(w1,p1),(w2,p2),(w3,p3)]))
                key = (triplet, total)
    ...
    # KEEP ONLY BEST PATTERNS (CRITICAL SPEED CONTROL)
...
# =====================================================
# OPTIMIZER (MATHEMATICALLY OPTIMAL)
# =====================================================
def optimize(A, b, total_mt):
    res = lsq_linear(A, b, bounds=(0, total_mt))
    x = res.x
...
# UI
# =====================================================
st.title("🧻 Optimized Deckle Planner (SciPy)")
...
st.write("Valid patterns:", len(patterns))
...
for pat, ton in zip(patterns, x):
    if ton > 0.01:
        st.write(f"{pat['widths']} → {pat['deckle']} mm | {ton:.2f} MT")
